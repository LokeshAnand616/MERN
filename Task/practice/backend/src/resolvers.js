...
//       try {
//         const { id, name, email, password, confirmPassword, role} = userData;
...
//         const newUser = new User({ id, name, email, password: hashedPassword, role });
//         await newUser.save();
  ...
  Mutation: {
    ...
    loginUser: async (_, { loginData }) => {
      try {
        ...
        if (isMatch) {
          const token = jwt.sign({ userName: user.name }, Secert, {
            expiresIn: "1h",
          });
          return { user, token };
        }
